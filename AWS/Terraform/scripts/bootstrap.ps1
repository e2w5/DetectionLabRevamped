...
If ($env:COMPUTERNAME -eq "DC") {
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.57.103    wef.windomain.local'
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.57.104    win11.windomain.local'
}
Else {
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.57.102    dc.windomain.local'
  Add-Content 'c:\\windows\\system32\\drivers\\etc\\hosts' '        192.168.57.102    windomain.local'
}

# Keep renewing the IP address until the domain controller is set as a DNS server
while (!(Get-DNSClientServerAddress | Where-Object { $_.ServerAddresses -eq "192.168.57.102" })) { 
