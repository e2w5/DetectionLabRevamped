...
post_build_checks() {
  ...
  SPLUNK_CHECK=$(curl -ks -m 2 https://192.168.57.105:8000/en-US/account/login?return_to=%2Fen-US%2F | grep -c 'This browser is not supported by Splunk')
  FLEET_CHECK=$(curl -ks -m 2 https://192.168.57.105:8412 | grep -c 'Fleet for osquery')
  ATA_CHECK=$(curl --fail --write-out "%{http_code}" -ks https://192.168.57.103 -m 2)
  VELOCIRAPTOR_CHECK=$(curl -ks -m 2 https://192.168.57.105:9999 | grep -c 'app/index.html')
  GUACAMOLE_CHECK=$(curl -ks -m 2 'http://192.168.57.105:8080/guacamole/#/' | grep -c 'Apache Software')
